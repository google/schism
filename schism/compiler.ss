          ...
          (let ((closure-var (gensym "closure-var")))
            `(let ((,closure-var (call %alloc (number ,(closure-tag))
               ...
               (seq ,(generate-save-free-vars
                      `(drop (icall %store-mem (var ,closure-var) (number 0)
  ...
  (define (generate-save-free-vars head closure free-vars index)
	...
	head
        (generate-save-free-vars
         `(seq ,head
               (drop (icall %store-mem (var ,closure) (number ,(* index (word-size)))
                            (var ,(car free-vars)))))
  ...
  (define (compile-expr expr env)
    (let ((tag (car expr)))
      (cond
       ((eq? tag 'nop) '(nop))
       ((eq? tag 'drop)
        (let ((compiled (compile-expr (cadr expr) env)))
          ;; FIXME: Remove this hack when we will be able to represent
          ;; void intrinsics directly.
          (if (and (eq? (car compiled) 'seq)
                   (eq? (caaddr compiled) 'i32.const))
              (cadr compiled)
              `(drop ,compiled))))
