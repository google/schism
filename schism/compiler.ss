...
(library (schism compiler)
      ...
      (if (function-definition? def)
          (let* ((args (function-formals def))
                ...
                (body (parse-body (function-body def)
                                  (add-lexicals args args* env))))
            `(,(cons name args*) ,body))
          `(define ,name ,(parse-expr (caddr def) env)))))
  ...
  (define (simplify-definition def)
    (let ((tag (car def)))
      (if (eq? tag 'define)
          ...
          `(,(car def) ,(simplify-expr (cadr def))))))
  ...
  (define (annotate-free-vars-expr expr bodies)
    (let ((tag (car expr)))
      (cond
       ...
       ((memq tag '(top var nop i32 const))
        expr)
       ((eq? tag 'lambda)
  ...
  (define (compile-library library)
    (let* ((libraries (read-imports library))
           ...
           ;; (dfs (map trace-value (cadr fns+dfs+globals+start)))
